<body>
<h1 style="text-align:center">บริษัท เมก้า คัตติ้งทูล จำกัด</h1>
<p style="text-align:center">รายงานแสดง Lot สินค้า(FIFO)<br>
พิมพ์ ณ วันที่ : 03/09/2019</p>
<p style="text-indent: 4em;">ตั้งแต่รหัสสินค้า 2ALE30200S06 ถึง ZYHOLDER04 , สิ้นสุด ณ วันที่ 03/09/2019 , เงื่อนไข : 02-แสดงเฉพาะมียอด </p>


<table align="center" width="1400" border="0" padding-top="0" cellspacing="0" style="border-top:solid 1px #000; text-align:center;">
<tr>
    <td style="border-bottom: solid 1px #000;"><b>วันที่</td>
    <td style="border-bottom: solid 1px #000;" width="200"><b>เอกสาร</td>
    <td style="border-bottom: solid 1px #000;"><b>คลัง</td>
    <td style="border-bottom: solid 1px #000;"><b>จำนวนรับ</td>
    <td style="border-bottom: solid 1px #000;"><b>จำนวนตัดคำนวณ</td>
    <td style="border-bottom: solid 1px #000;"><b>คงเหลือ</td>
    <td style="border-bottom: solid 1px #000;"><b>หน่วย</td>
    <td style="border-bottom: solid 1px #000;"><b>มูลค่าคงเหลือ</td>
    <td style="border-bottom: solid 1px #000;"><b>ต้นทุน/หน่วย</td>
  </tr>
  @foreach ($Product as $Product)
  <tr height="40">
    <td>สินค้า</td>
    <td>{{$Product->product_code}}</td>
  
  </tr>
  @foreach ($Product->gaurd_stock as $gaurd_stock)
  <tr height="40">
    <td>11/3/2019</td>
    <td>AJ6203-00023</td>
    <td>01</td>
    <td>10.00</td>
    <td>0.00</td>
    <td>10.00</td>
    <td>ชิ้น</td>
    <td>1,640.00</td>
    <td>164.00</td>
  </tr>
  @endforeach

  @endforeach
  
</table>
</div>
</body>
