...
<form action="{{url('/')}}/supplier/{{$row->supplier_id}}" method="POST" id="form" enctype="multipart/form-data" class="mt-4">
  {{ csrf_field() }}
  {{ method_field('PUT') }}
  ...
  @endphp
  @include('supplier/contact-index-card', ['type' => 'supplier'])

  @php
  $comment = $supplier->comments;
  @endphp
  @include('supplier/comment-index-card', ['type' => 'supplier'])
