<div class="card">
  <div class="card-block">
    <div class="row text-center">
      <div class="col-6 col-md-4 ">
        <a class="btn btn-outline-primary btn-menu  my-3" href="{{url('/')}}/finance/customer-debt?debt_type=ตั้งหนี้คงค้าง">
          <div class="vertical-center">
            <div class="px-4"><i class="round round-primary my-2 ni ni-single-copy-04"></i></div>
            <div>ตั้งหนี้<br>คงค้าง</div>
          </div>
        </a>   
      </div>
      <div class="col-6 col-md-4 ">
        <a class="btn btn-outline-primary btn-menu  my-3" href="{{url('/')}}/finance/customer-debt?debt_type=ตั้งหนี้ลูกหนี้">
          <div class="vertical-center">
            <div class="px-4"><i class="round round-primary my-2 ni ni-single-copy-04"></i></div>
            <div>ตั้งหนี้/<br>ลูกหนี้</div>
          </div>
        </a>   
      </div>
      <div class="col-6 col-md-4 ">
        <a class="btn btn-outline-primary btn-menu  my-3" href="{{url('/')}}/finance/customer-debt?debt_type=ลดหนี้ลูกหนี้">
          <div class="vertical-center">
            <div class="px-4"><i class="round round-primary my-2 ni ni-single-copy-04"></i></div>
            <div>ลดหนี้/<br>ลูกหนี้</div>
          </div>
        </a> 
      </div>
      <div class="col-6 col-md-4 ">
      <a class="btn btn-outline-primary my-4 m" href="{{ url('/') }}/finance/billing">
          <div class="px-4"><span class="round round-primary my-2">บ</span></i></div>
          <div><br>ใบวางบิล</div>
        </a>
      </div>
      <div class="col-6 col-md-4 ">
        <a class="btn btn-outline-primary my-4" href="#">
          <div class="px-4"><span class="round round-primary my-2">ช</span></div>
          <div><br>ชำระเงิน</div>
        </a>
      </div>
      <div class="col-6 col-md-4 ">
        <a class="btn btn-outline-success btn-menu  my-3" href="{{url('/')}}/customer">
          <div class="vertical-center">
            <div class="px-4"><i class="round round-success my-2 ni ni-folder-17"></i></div>
            <div>แฟ้มหลัก<br>ลูกค้า</div>
          </div>
        </a>          
      </div>
    </div>
  </div>
</div>
