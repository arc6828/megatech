		...
		$('#customerModal').on('show.bs.modal', function (e) {
					...
					dataType : "json",
						...
						result.forEach(function(element,index) {
							...
							let row = [
								...
								element.company_name+"<input type='hidden' class='customer_id' value='"+element.customer_id+"'>",
						...
						$('#table-customer-modal').on( 'click', 'tr', function () {
							...
							console.log(d);
							console.log("ROW : ",d, $("<div>"+d[1]+"</div>").find(".customer_id"));
							let customer_id = $("<div>"+d[1]+"</div>").find(".customer_id")[0].value;
			...
			.done(function(result){
				...
				result.forEach(function(element,index) {
					...
					let row = [
						"<a href='javascript:void(0)' onclick='fillInvoice("+order_id+");'>" + element.order_code + "</a>",
						//element.date,
