<div class="card mb-4">
    <div class="card-body text-center pr-5 pl-1">
                ...
                <div class="col-lg-4">
                    @if (isset($mode))
                        <div class="text-center"><img
                                src="data:image/png;base64,{{ DNS1D::getBarcodePNG($returninvoice->code, 'C128') }}"
                                alt="barcode" /></div>
                    ...
                    @if (isset($mode))
                        @if ($mode == 'show')
                            @if ($returninvoice->sales_status_id != -1)
                                <a class="btn btn-sm btn-warning btn-print mr-2"
                                    href="{{ url('/') }}/sales/return-invoice/{{ $returninvoice->id }}/edit"
                                    title="พิมพ์">
                                    <i class="fas fa-edit"></i> แก้ไข
                                </a>
                            @elseif($returninvoice->sales_status_id == 15)
                        ...
                        @elseif($mode=="edit")
                            @if (isset($returninvoice->sales_status_id))
                                @if ($returninvoice->sales_status_id == 6)
                        ...
                        <a class="btn btn-primary btn-sm btn-print mr-2"
                            href="{{ url('/') }}/sales/return-invoice/{{ $returninvoice->id }}/pdf"
        ...
        <div class="form-group form-row">
            ...
            <input class="form-control form-control-sm  col-lg-3" name="code" type="text" id="code"
                value="{{ isset($returninvoice->code) ? $returninvoice->code : '' }}" readonly>
            <label for="customer_id" class="control-label  col-lg-3">{{ 'วันที่เวลา' }}</label>
            <input class="form-control form-control-sm  col-lg-3" name="" type="" id=""
                value="{{ isset($returninvoice->created_at) ? $returninvoice->created_at : '' }}" readonly>
        </div>
        <div class="form-group form-row">
            <label for="customer_id" class="control-label  col-lg-3">{{ 'รหัสลูกหนี้' }}</label>
            <input class="form-control form-control-sm  col-lg-3" name="customer_id" type="hidden" id="customer_id"
                value="{{ isset($returninvoice->customer_id) ? $returninvoice->customer_id : '' }}">
            ...
            <div class="input-group input-group-sm col-lg-3 ">
                ...
                <input class="form-control"
                    value="{{ isset($returninvoice->customer_id) ? $returninvoice->customer->company_name : '' }}"
                    readonly>
                @if (!isset($mode))
                    <div class="input-group-append">
                        <button class="btn btn-success" id="btn-customer" type="button" data-toggle="modal"
            ...
            <input class="form-control form-control-sm  col-lg-3" name="invoice_code" type="text" id="invoice_code"
                value="{{ isset($returninvoice->invoice_code) ? $returninvoice->invoice_code : '' }}" readonly>
        </div>
        <div class="form-group  form-row">
            <label for="tax_type_id" class="control-label   col-lg-3">{{ 'ชนิดภาษี' }}</label>
            <input class="form-control form-control-sm   col-lg-3" name="tax_type_id" type="hidden" id="tax_type_id"
                value="{{ isset($returninvoice->tax_type_id) ? $returninvoice->tax_type_id : '' }}">
            <input class="form-control form-control-sm   col-lg-3" type="text"
                value="{{ isset($returninvoice->tax_type_id) ? $returninvoice->tax_type->tax_type_name : '' }}"
                readonly>

            <label for="sales_status_id" class="control-label   col-lg-3">{{ 'สถานะ' }}</label>
            <input class="form-control form-control-sm   col-lg-3" name="sales_status_id" type="hidden"
                id="sales_status_id"
                value="{{ isset($returninvoice->sales_status_id) ? $returninvoice->sales_status_id : '' }}">
            <div class="col-lg-3">
                @if (isset($returninvoice->sales_status_id))
                    @switch($returninvoice->sales_status_id)
                        @case(-1)
                            <span
                                class="badge badge-pill badge-secondary">{{ $returninvoice->sales_status->sales_status_name }}</span>
                        @break
                        @case(6)
                            <span
                                class="badge badge-pill badge-primary">{{ $returninvoice->sales_status->sales_status_name }}</span>
                        @break
                        {{-- @case(" 15")
                            <span
                                class="badge badge-pill badge-success">{{ $returninvoice->sales_status->sales_status_name }}</span>
                        @break --}}
                        @default
            ...
            <div class="form-group  form-row">
                ...
                <input class="form-control form-control-sm   col-lg-3" name="staff_id" type="hidden" id="staff_id"
                    value="{{ isset($returninvoice->staff_id) ? $returninvoice->staff_id : '' }}">

                <input class="form-control form-control-sm   col-lg-3" type="text"
                    value="{{ isset($returninvoice->staff_id) ? $returninvoice->staff->name : '' }}" readonly>

                <label for="user_id" class="control-label   col-lg-3">{{ 'พนักงานผู้บันทึก' }}</label>
                <input class="form-control form-control-sm   col-lg-3" name="user_id" type="hidden" id="user_id"
                    value="{{ isset($returninvoice->user_id) ? $returninvoice->user_id : '' }}">

                <input class="form-control form-control-sm   col-lg-3" type="text"
                    value="{{ isset($returninvoice->user_id) ? $returninvoice->user->name : '' }}" readonly>
            </div>
        </div>
    </div>
    <div class="card mb-4">
            ...
            <div class="form-row">
                <div class="form-group {{ $errors->has('remark') ? 'has-error' : '' }} col-lg-6">
                    ...
                    <textarea class="form-control form-control-sm" rows="5" name="remark" type="textarea"
                        id="remark">{{ isset($returninvoice->remark) ? $returninvoice->remark : '' }}</textarea>
                ...
                <div class="col-lg-5">
                    <div class="form-group {{ $errors->has('total_before_vat') ? 'has-error' : '' }} form-row">
                        ...
                        <input class="form-control form-control-sm col-lg-6" name="total_before_vat" type="number"
                            id="total_before_vat"
                            value="{{ isset($returninvoice->total_before_vat) ? $returninvoice->total_before_vat : '' }}"
                            readonly>
                    ...
                    <div class="form-group {{ $errors->has('vat') ? 'has-error' : '' }} form-row">
                        ...
                        <input class="form-control form-control-sm col-lg-2" name="vat_percent" type="number"
                            id="vat_percent"
                            value="{{ isset($returninvoice->vat_percent) ? $returninvoice->vat_percent : '' }}" readonly>
                        ...
                        <input class="form-control form-control-sm col-lg-6" name="vat" type="number" id="vat"
                            value="{{ isset($returninvoice->vat) ? $returninvoice->vat : '' }}" readonly>
                    ...
                    <div class="form-group {{ $errors->has('total_after_vat') ? 'has-error' : '' }} form-row">
                        ...
                        <input class="form-control form-control-sm col-lg-6" name="total_after_vat" type="number"
                            id="total_after_vat"
                            value="{{ isset($returninvoice->total_after_vat) ? $returninvoice->total_after_vat : '' }}"
                            readonly>
