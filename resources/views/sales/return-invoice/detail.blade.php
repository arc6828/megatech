    ...
    <table class="table table-sm text-center" width="100%">
        <thead>
            <tr>
                ...
                <th>ราคาที่ขาย</th>
                <th>รวม</th>
                <th class="d-none">Return Invoice Id</th>
                <!-- <th>Actions</th> -->
            </tr>
        </thead>
        <tbody>
            @php
                $returninvoicedetail = isset($returninvoicedetail) ? $returninvoicedetail : [];
            @endphp
                ...
                <tr>
                    ...
                    @if (isset($mode))
                        <td><input type="number" class="input amounts" name="amounts[]" value="{{ $item->amount }}"
                                min="0" max="{{ $item->amount }}" title="[0,{{ $item->amount }}]"
                                style="max-width:60px;" /> / {{ $item->amount }}</td>
                        <td><input type="hidden" class="input discount_prices" name="discount_prices[]"
                                value="{{ $item->discount_price }}" /> {{ number_format($item->discount_price, 2) }}
                        </td>
                        <td><input type="number" class="input totals" name="totals[]"
                                value="{{ $item->amount * $item->discount_price }}" style="max-width:100px;"
                                readonly /></td>
                    @else
                        <td><input type="number" class="input amounts" name="amounts[]" value="0" min="0"
                                max="{{ $item->amount }}" title="[0,{{ $item->amount }}]"
                                style="max-width:60px;" /> / {{ $item->amount }}</td>
                        <td><input type="hidden" class="input discount_prices" name="discount_prices[]"
                                value="{{ $item->discount_price }}" />
                            {{ number_format($item->discount_price, 2) }}</td>
                        <td><input type="number" class="input totals" name="totals[]" value="0" style="max-width:100px;"
                                readonly /></td>

                    @endif
                    <td class="d-none">{{ $item->return_invoice_id }}</td>
                    <!-- <td>
                    <a href="{{ url('/sales/return-invoice-detail/' . $item->id) }}" title="View ReturnInvoiceDetail"><button class="btn btn-info btn-sm"><i class="fa fa-eye" aria-hidden="true"></i> View</button></a>
                    <a href="{{ url('/sales/return-invoice-detail/' . $item->id . '/edit') }}" title="Edit ReturnInvoiceDetail"><button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit</button></a>

                    <form method="POST" action="{{ url('/sales/return-invoice-detail' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                        {{ method_field('DELETE') }}
                        {{ csrf_field() }}
