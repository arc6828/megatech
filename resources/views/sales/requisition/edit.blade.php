...
@forelse($table_requisition as $row)
<form id="form-delete" style="display: none;" action="{{ url('/') }}/sales/requisition/{{ $row->requisition_id }}" method="POST">
	{{ csrf_field() }}
	{{ method_field('DELETE') }}
...
<form class="" action="{{ url('/') }}/sales/requisition/{{ $row->requisition_id }}" method="POST">
	{{ csrf_field() }}
	{{ method_field('PUT') }}
	<div class="card">
		<div class="card-block">
			...
			<div class="col-lg-9 align-self-center">
				<h4 class="card-title">Requisition code : {{ $row->requisition_code }}</h4>
	...
	@include('sales/requisition_detail/table')
...
<div style="display:none;">
	...
	<script>
		function onDelete(id){
			...
			form.action = "{{ url('/') }}/sales/requisition/{{ $requisition_id }}/requisition_detail/"+id;
			//SUBMIT
			var want_to_delete = confirm('Are you sure to delete this requisition detail?');
			if(want_to_delete){
				form.submit();
			}
		}
