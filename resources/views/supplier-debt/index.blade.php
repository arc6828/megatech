                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="table">
                                <thead>
                                    <tr>
                                        <th>#</th><th>Doc No</th><th>Date</th><th>Supplier Id</th><th>Discount</th><th>Amount</th><th>Vat Percent</th><th>Vat</th><th>Total Before Vat</th><th>Total</th><th>Tax Type Id</th><th>Completed At</th><th>Tax Category</th><th>Round</th><th>Type Debt</th><th>Debt Duration</th><th>User Id</th><th>Role</th><th>Reference</th><th>Zone Id</th><th>Zone Id</th><th>Cheque Id</th><th>Payment Method</th><th>Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                @foreach($supplierdebt as $item)
                                    <tr>
                                        ...
                                        <td>{{ $item->doc_no }}</td><td>{{ $item->date }}</td><td>{{ $item->supplier_id }}</td><td>{{ $item->discount }}</td><td>{{ $item->amount }}</td><td>{{ $item->vat_percent }}</td><td>{{ $item->vat }}</td><td>{{ $item->total_before_vat }}</td><td>{{ $item->total }}</td><td>{{ $item->tax_type_id }}</td><td>{{ $item->completed_at }}</td><td>{{ $item->tax_category }}</td><td>{{ $item->round }}</td><td>{{ $item->type_debt }}</td><td>{{ $item->debt_duration }}</td><td>{{ $item->user_id }}</td><td>{{ $item->role }}</td><td>{{ $item->reference }}</td><td>{{ $item->zone_id }}</td><td>{{ $item->zone_id }}</td><td>{{ $item->cheque_id }}</td><td>{{ $item->payment_method }}</td>
                                        <td>
                            ...
                            <div class="pagination-wrapper"> {!! $supplierdebt->appends(['search' => Request::get('search')])->render() !!} </div>
                        </div>
