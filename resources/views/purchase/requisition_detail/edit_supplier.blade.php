...
@section('content')
            ...
            <form method="get" action="">
                <div class="form-group form-inline">
                    ...
                    <div class="col-lg-3">
                        <select name="purchase_requisition_detail_status_id" id="purchase_requisition_detail_status_id"
                            class="form-control form-control-sm" onchange="onSubmit(this);" required>
                            ...
                            @foreach ($table_purchase_requisition_detail_status->where('purchase_requisition_detail_status_id', '1') as $row_purchase_requisition_detail_status)
                                <option
                                    class="{{ in_array($row_purchase_requisition_detail_status->purchase_requisition_detail_status_name, ['อนุมัติ', 'ไม่อนุมัติ']) ? '' : 'd-none' }}"
                ...
                <div class="form-group form-inline d-none">
                    ...
                    <div class="col-lg-2 d-none">
                        <select name="m_date" id="m_date" class="form-control form-control-sm" onchange="onSubmit(this);"
                            style="max-width:150px;">
                            <option value="">None</option>

                            @for ($i = 0; $i <= 12; $i++)
                                <option value="{{ date('Y-m-01', strtotime(date('Y-m-01') . " -$i months")) }}"
    ...
    <div class="mt-4 card">
        <div class="card-body">
            <form action="{{ url('/') }}/purchase/requisition_detail/update_supplier" method="post" id="form_table"
                onsubmit="return validateCheckbox();">
                {{ csrf_field() }}
                {{ method_field('PUT') }}

                <div class="table-responsive">
                    <table class="table table-hover text-center table-sm" id="table-purchase_requisition-detail"
                        style="width:100%">
                ...
                <div class="form-group form-inline text-center">
                    <div class="col-lg-12">
                        <select name="action" id="action" class="form-control form-control-sm" required>
                            @foreach ($table_supplier as $row_supplier)
                                <option value="{{ $row_supplier->supplier_id }}">
                                    {{ $row_supplier->supplier_code }} - {{ $row_supplier->company_name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <button type="submit" id="form_submit_table" class="btn btn-success btn-sm ml-2">
                            submit
                        </button>
                    </div>
                </div>
            ...
            document.addEventListener("DOMContentLoaded", function(event) {
                ...
                $.ajax({
                    ...
                    result.forEach(function(element, index) {
                        ...
                        let displayCheckbox = (element.purchase_requisition_detail_status_id ==
                            '4') ? "class='d-none'" : "";
                        console.log(element, index, displayCheckbox);

                        var row = [
                            "<input type='checkbox' " + displayCheckbox +
                            " name='selected_purchase_requisition_detail_ids[]'  value='" + id +
                            "' >" +
                            "<input type='hidden' name='purchase_requisition_detail_ids[]' value='" +
                            id + "' >" +
                            "<input type='hidden' name='amounts[]' value='" + element.amount +
                            "'  >" +
                            "<span class='text-danger' title='" + element.company_name + "'>" +
                            (element.supplier_id ? element.supplier_code : "") + "</span>",
                            ...
                            (element.supplier_id ? "readonly" : "") + "  required>",
                            0,
                            0,
                            0,
                        ];
                    ...
                    //DATA TABLE SCROLL
                    var tableCont = document.querySelector('#table-purchase_requisition-detail');
