...
@section('content')
                    ...
                    <div class="col-lg-3">
                        <select name="purchase_requisition_detail_status_id" id="purchase_requisition_detail_status_id"
                            ...
                            @foreach ($table_purchase_requisition_detail_status as $row_purchase_requisition_detail_status)
                                <option
                                    class="{{ in_array($row_purchase_requisition_detail_status->purchase_requisition_detail_status_name, ['อนุมัติ', 'ไม่อนุมัติ', 'รออนุมัติ']) ? '' : 'd-none' }}"
    ...
    <div class="mt-4 card">
        <div class="card-body">
            <form action="{{ url('/') }}/purchase/requisition_detail/approve" method="post" id="form_table"
                ...
                $.ajax({
                    ...
                    result.forEach(function(element, index) {
                        ...
                        var willing_amount = parseInt(element.pending_out) - (parseInt(element
                            .amount_in_stock) + parseInt(element.pending_in));
                        if (willing_amount < 0) {
                            willing_amount = 0;
                        }
                        ...
                        var row = [
                            "<input type='checkbox' name='selected_purchase_requisition_detail_ids[]' value='" +
                            id + "' >" +
