...
@section('title','Returnorderdetail')
...
@section('content')
    <div class="container">
        <div class="row">
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">Returnorderdetail</div>
                    <div class="card-body">
                        <a href="{{ url('/purchase/return-order-detail/create') }}" class="btn btn-success btn-sm" title="Add New ReturnOrderDetail">
                            <i class="fa fa-plus" aria-hidden="true"></i> Add New
                        </a>

                        <form method="GET" action="{{ url('/purchase/return-order-detail') }}" accept-charset="UTF-8" class="form-inline my-2 my-lg-0 float-right" role="search">
                        ...
                        <div class="table-responsive">
                            <table class="table">
                                <thead>
                                    <tr>
                                        <th>#</th><th>Product Id</th><th>Amount</th><th>Discount Price</th><th>Total</th><th>Return Order Id</th><th>Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                @foreach($returnorderdetail as $item)
                                    <tr>
                                        ...
                                        <td>{{ $item->product_id }}</td><td>{{ $item->amount }}</td><td>{{ $item->discount_price }}</td><td>{{ $item->total }}</td><td>{{ $item->return_order_id }}</td>
                                        <td>
                            ...
                            <div class="pagination-wrapper"> {!! $returnorderdetail->appends(['search' => Request::get('search')])->render() !!} </div>
                        </div>
