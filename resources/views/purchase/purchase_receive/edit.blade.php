...
<form id="form-delete" style="display: none;" action="{{ url('/') }}/purchase/purchase_receive/{{ $row->purchase_receive_id }}" method="POST">
	{{ csrf_field() }}
	{{ method_field('DELETE') }}
...
<form class="" action="{{ url('/') }}/purchase/purchase_receive/{{ $row->purchase_receive_id }}" method="POST">
	{{ csrf_field() }}
	{{ method_field('PUT') }}
	<div class="card">
