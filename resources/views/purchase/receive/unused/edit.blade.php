...
@section('title','แก้ไขรายละเอียดใบซื้อ/รับสินค้า')

@section('content')

@forelse($table_purchase_receive as $row)
<form id="form-delete" style="display: none;" action="{{ url('/') }}/purchase/receive/{{ $row->purchase_receive_id }}" method="POST">
	{{ csrf_field() }}
	{{ method_field('DELETE') }}
...
<form class="" action="{{ url('/') }}/purchase/receive/{{ $row->purchase_receive_id }}" method="POST">
	{{ csrf_field() }}
	{{ method_field('PUT') }}
	<div class="card">
		<div class="card-body">
			...
			<div class="col-lg-9 align-self-center">
				<h4 class="card-title">Purchase Receive code : {{ $row->purchase_receive_code }}</h4>
	...
	@include('purchase/receive_detail/table')
				...
				<div class="col-lg-12">
					...
					<div class="form-group form-inline">
						...
						<label class="col-lg-3">
							ภาษีมูลค่าเพิ่ม
							<input type="number" name="vat_percent"  id="vat_percent" value="{{ $row->vat_percent }}" onkeyup="onChange(this)" onChange="onChange(this)" class="form-control form-control-line"  style="width: 50px; margin: 10px;"> %
						</label>
						<div class="col-lg-3">
...
<div style="display:none;">
	...
	<script>
		function onDelete(id){
			...
			form.action = "{{ url('/') }}/purchase/receive/{{ $purchase_receive_id }}/purchase_receive_detail/"+id;
			//SUBMIT
