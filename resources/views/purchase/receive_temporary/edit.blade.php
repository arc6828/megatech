        ...
        @if ($mode == 'edit')
            ...
            @section('content')
                ...
                @forelse($table_receive_temporary as $row)
                    <form class="d-none"
                        action="{{ url('/') }}/purchase/receive_temporary/{{ $row->receive_temporary_id }}/cancel"
                        id="form-cancel" method="POST" onsubmit="return confirm('Do you confirm to cancel?')">
                        {{ csrf_field() }}
                        {{ method_field('PATCH') }}
                        <button type="submit" class="btn btn-primary " id="form-cancel-submit" style="width:150px;">Save</button>
                    </form>
                    <form class="d-none"
                        action="{{ url('/') }}/purchase/receive_temporary/{{ $row->receive_temporary_id }}/approve"
                        id="form-approve" method="POST" onsubmit="return confirm('ต้องการเปิดใบรับของชั่วคราว ?')">
                        {{ csrf_field() }}
                        {{ method_field('PUT') }}
                        <button type="submit" class="btn btn-success " id="form-approve-submit" style="width:150px;">Save</button>
                    </form>

                    {{-- <form method="POST" action="{{ url('/') }}/purchase/receive_temporary/{{ $row->receive_temporary_id }}"
                    ...
                    @if ($row->purchase_status_id == 5)
                        <form class=""
                            action="{{ url('/') }}/purchase/receive_temporary/{{ $row->receive_temporary_id }}/update"
                            ...
                            @if ($row->purchase_status_id != 11)
                                @if ($mode == 'edit')
                                    <div class="text-center mt-4">
                                        <a href="{{ url('/') }}/purchase/receive_temporary" class="btn btn-outline-success"
                    ...
                    @elseif($row->purchase_status_id == 10)
                        <form class=""
                            action="{{ url('/') }}/purchase/receive_temporary/{{ $row->receive_temporary_id }}/revision"
                            id="form" method="POST" onsubmit="return confirm('Do you confirm to save?')">
                            {{ csrf_field() }}
                            {{ method_field('PUT') }}
                    ...
                    <script>
                        document.addEventListener("DOMContentLoaded", function(event) {
                            $(".btn-print").attr("href",
                                "{{ url('/') }}/purchase/receive_temporary/{{ $row->receive_temporary_id }}");
