<div class="card">
    <div class="card-body">
        <div class="row mb-4">
            <div class="col-lg-4">
                <a href="{{ url('/purchase/receive_temporary') }}" title="Back" class="btn btn-warning btn-sm">
                    <i class="fa fa-arrow-left" aria-hidden="true"></i> Back
                </a>
            </div>
            @if (isset($table_receive_temporary))
                <div class="col-lg-4 text-center">
                    <div class="">
                        <img src="data:image/png;base64,{{ DNS1D::getBarcodePNG($row->receive_temporary_code, 'C128') }}"
                            alt="barcode" />
                    </div>
                    <div class="">
                ...
                <div class="col-lg-4 text-right">
                    @if ($mode == 'edit')
                        @if ($row->purchase_status_id == 5)
                            <a class="px-2 btn btn-sm btn-success" href="javascript:void(0)" onclick="approved()"
                                title="สร้าง">
                                <i class="fas fa-check"></i> สร้างใบรับของชั่วคราว
                            </a>
                            {{-- <a class="px-2 btn btn-sm btn-danger" href="javascript:void(0)"
                                onclick="document.querySelector('#form-delete-submit').click();" title="ลบทิ้ง">
                                <i class="fas fa-trash"></i> ลบทิ้ง
                            </a> --}}
                        @endif

                        @if ($row->purchase_status_id == 10)
                            <a class="px-2 btn btn-sm btn-success" href="javascript:void(0)"
                                onclick="document.querySelector('#form-cancel-submit').click(); console.log(5565);"
                                title="ยกเลิก">
                                <i class="fas fa-trash"></i> ปิดใบรับของชั่วคราว
                            </a>
                        @endif
                    @elseif($mode == "show")
                        @if ($row->purchase_status_id != 11)
                            <a class="px-2 btn btn-sm btn-warning"
                                href="{{ url('/') }}/purchase/receive_temporary/{{ $row->receive_temporary_id }}/edit"
                                title="แก้ไข">
                    ...
                    <a class="px-2 btn btn-sm btn-primary"
                        href="{{ url('/') }}/purchase/receive_temporary/{{ $row->receive_temporary_id }}/pdf"
        ...
        <div class="form-group form-inline">
            ...
            <div class="col-lg-3">
                <input type="hidden" name="receive_temporary_id"
                    value="{{ isset($receive_temporary->receive_temporary_id) ? $receive_temporary->receive_temporary_id : '' }}" />
                <input name="receive_temporary_code" id="receive_temporary_code" class="form-control form-control-sm"
                    readonly>
            </div>
        ...
        <div class="form-group form-inline">
            ...
            @include('purchase/receive_temporary/supplier_modal')
        ...
        <div class="form-group form-inline d-none">
            ...
            <label class="col-lg-2 offset-lg-1">ระยะเวลารับของ (วัน)(deplicated)</label>
            <div class="col-lg-3">
                <input type="number" name="receive_time" id="receive_time"
                    class="form-control form-control-sm form-control-line" disabled>
            </div>
        ...
        <div class="form-group form-inline">
            ...
            <div class="col-lg-3">
                <select name="tax_type_id" id="tax_type_id" class="form-control form-control-sm"
                    onChange="onChange(this)" required readonly>
            ...
            <div class="col-lg-3">
                <select name="receive_type_id" id="receive_type_id" class="form-control form-control-sm" required
                    readonly>

                    @foreach ($table_receive_type as $row_receive_type)
                        <option value="{{ $row_receive_type->delivery_type_id }}">
                            {{ $row_receive_type->delivery_type_name }}
                        </option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="form-group form-inline">
            ...
            <div class="col-lg-3">
                <input type="text" name="purchase_status_id" id="purchase_status_id"
                    class="d-none form-control form-control-sm form-control-line"
                    value="{{ isset($receive_temporary) ? $receive_temporary->purchase_status_id : '0' }}" readonly>
                <input type="text" name="" id="" class="d-none form-control form-control-sm form-control-line "
                    value="{{ isset($receive_temporary) ? $receive_temporary->purchase_status->purchase_status_name : '0' }}"
                    readonly>
                @if (isset($receive_temporary))
                    @switch($receive_temporary->purchase_status_id)
                        @case(-1)
                            <span
                                class="badge badge-pill badge-secondary">{{ $receive_temporary->purchase_status->purchase_status_name }}</span>
                        @break
                        @case(5)
                            <span
                                class="badge badge-pill badge-primary">{{ $receive_temporary->purchase_status->purchase_status_name }}</span>
                        @break
                        @case(10)
                            <span
                                class="badge badge-pill badge-warning">{{ $receive_temporary->purchase_status->purchase_status_name }}</span>
                        @break
                        @case(11)
                            <span
                                class="badge badge-pill badge-success">{{ $receive_temporary->purchase_status->purchase_status_name }}</span>
                        @break
                    @endswitch
                @endif
            </div>
...
@include('purchase/receive_temporary/detail')
...
<script>
    function refreshTotal() {
        var total = 0;
        document.querySelectorAll("#table-receive_temporary-detail .total_edit").forEach(function(element, index) {
