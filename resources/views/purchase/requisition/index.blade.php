                ...
                <div class="col-lg-6 align-self-center">
                    <form class="" action="{{ url('/') }}/purchase/requisition" method="GET">
                        <div class="form-group form-inline pull-right">
                            <input class="form-control mb-2 mr-2" type="text" name="q" placeholder="type your keyword..."
                                value="{{ $q }}">
                    ...
                    <tbody>
                        @foreach ($table_purchase_requisition as $row)
                            ...
                            <tr>
                                <td>
                                    <a
                                        href="{{ url('/') }}/purchase/requisition/{{ $row->purchase_requisition_id }}">
                                ...
                                <td>
                                    @switch($row->purchase_status_id)
                                        ...
                                        @default
                                            ...
                                            @elseif( $row->requisition_details->sum("purchase_requisition_detail_status_id") ==
                                                $row->requisition_details->count("purchase_requisition_detail_status_id")*3 )
                                                <span class="badge badge-pill badge-info">รอการอนุมัติ</span>
                                            @elseif( $row->requisition_details->sum("purchase_requisition_detail_status_id") ==
                                                $row->requisition_details->count("purchase_requisition_detail_status_id")*2 )
