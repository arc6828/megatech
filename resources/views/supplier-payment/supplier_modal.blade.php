					...
					<table class="table table-hover text-center table-sm" id="table">
						<thead>
							<tr>
								...
								<th class="text-center">ยอดหนึ้ที่วางบิล</th>
						...
						@php
							...
							$total_debt_billing = $row->Receives_on_debt_billing->sum('total_debt');
						...
						<tr>
							...
							<td class=" ">
								@if(false)
								<a class="btn btn-sm btn-primary" href="{{ url('/') }}/finance/supplier-billing/create?supplier_id={{ $row->supplier_id }}&end_date={{ date('Y-m-t', strtotime( '-1 month' ) ) }}">วางบิลเดือนที่ผ่านมา ({{ number_format($total_debt_last_cycle,2) }})</a>
								<a class="btn btn-sm btn-success" href="{{ url('/') }}/finance/supplier-billing/create?supplier_id={{ $row->supplier_id }}&end_date={{ date('Y-m-t' ) }}">วางบิลทั้งหมด ({{ number_format($total_debt,2) }})</a>
								@endif
								...
								@if($total_debt > 0)
								<a class="btn btn-sm btn-primary" href="{{ url('/') }}/finance/supplier-payment/create?supplier_id={{ $row->supplier_id }}&filter=billing-only">รับชำระที่วางบิล</a>
								<a class="btn btn-sm btn-success" href="{{ url('/') }}/finance/supplier-payment/create?supplier_id={{ $row->supplier_id }}">รับชำระทั้งหมด</a>
								@endif
								<a href="javascript:void(0)" onclick="onDelete( {{ $row->supplier_id }} )" class="text-danger d-none">
									<span class="fa fa-trash"></span>
								</a>
