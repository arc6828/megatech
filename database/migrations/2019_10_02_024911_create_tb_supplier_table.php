...
class CreateTbSupplierTable extends Migration {
	...
	public function up()
	{
		...
		{
			$table->integer('supplier_id', true)->comment('id');
			$table->string('supplier_code', 100)->nullable();
			$table->string('supplier_type', 100)->nullable()->comment('ประเภทเจ้าหนี้');
			...
			$table->integer('supplier_name')->nullable()->default(555);
			...
			$table->string('tax_number', 13)->nullable()->comment('เลขที่ภาษี');
			$table->string('billing_duration')->nullable()->comment('เงื่อนไขวางบิล');
			$table->string('cheqe_condition')->nullable();
			$table->string('delivery_time', 20)->nullable();
			$table->string('payment_condition')->nullable()->comment('เงื่อนไขรับเช็ค');
			$table->integer('tax_type_id')->nullable()->default(2);
